...
namespace CodeStage.AntiCheat.EditorCode
{
	...
	internal static class EditorTools
	{
		...
		public static void TraverseSerializedScriptsInScenes(ProcessSerializedProperty itemCallback, string[] typesFilter, bool skipSave = false)
		{
			var touchedCount = 0;
			var scannedCont = 0;
		...
		private static bool GetScriptingObjects(string filter, bool loadAll, ref Dictionary<Object, AssetLocationData> results)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (loadAll)
				{
					...
					foreach (var unityObject in objects)
					{
						if (unityObject == null) continue;
						if (unityObject.name == "Deprecated EditorExtensionImpl") continue;
						if (results.ContainsKey(unityObject)) continue;
						results.Add(unityObject, AssetTools.GetLocation(path, unityObject));
						anythingFound = true;
					}
				}
				else
				{
					...
					foreach (var component in components)
					{
						...
						if (results.ContainsKey(component)) continue;
						results.Add(component, AssetTools.GetLocation(path, component));
						anythingFound = true;
					}
				}
			}
